...
{
    const uint32_t LONG_WORK_ITERATIONS = 5000000; // Valor a ajustar per al temps desitjat
    while (1) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            // Bucle que només consumeix CPU. LOW (ara P7) NO pot ser interrompuda per MEDIUM (P5).
            for (uint32_t j = 0; j < LONG_WORK_ITERATIONS; j++) {
                __asm__ __volatile__ ("nop"); // O qualsevol càlcul senzill
            }
